...
public class BoardFrame extends WindowBase {
   ...
   * menu frame. If p_option = SINGLE_FRAME, this frame is created directly a
   * single frame. If
   * p_option = Option.IN_SAND_BOX, no security sensitive actions like for example
   * choosing If
   * p_option = Option.WEBSTART, the application has been started with Java
   * Webstart. files are
   * allowed, so that the frame can be used in an applet. Currently,
   * Option.EXTENDED_TOOL_BAR is used
   * only if a new board is created by the application from scratch. If
   * p_test_level {@literal >}
   * RELEASE_VERSION, functionality not yet ready for release is included. Also,
   * the warning output
   * depends on p_test_level.
   */
  public BoardFrame(DesignFile p_design, Locale p_locale, boolean p_save_intermediate_stages,
      float p_optimization_improvement_threshold, DisabledFeaturesSettings disabledFeatures) {
    this(p_design, new BoardObserverAdaptor(), p_locale, p_save_intermediate_stages,
        p_optimization_improvement_threshold, disabledFeatures);
  }

  /**
   * Creates new form BoardFrame. The parameters p_item_observers and
   * p_item_id_no_generator are
   * used for synchronizing purposes, if the frame is embedded into a host system,
   */
  BoardFrame(DesignFile p_design, BoardObservers p_observers, Locale p_locale, boolean p_save_intermediate_stages,
      float p_optimization_improvement_threshold, DisabledFeaturesSettings disabledFeatures) {
    super(800, 150);
    ...
    this.menubar.fileMenu.addSaveAsEventListener((File selectedFile) -> {
      ...
      switch (p_design.outputFileFormat) {
        case SES:
          ...
          boolean sesFileSaved = this.saveAsSpecctraSessionSes(this.design_file.getOutputFile(),
    ...
    this.screen_messages = new ScreenMessages(this.message_panel.errorLabel, this.message_panel.warningLabel,
        this.message_panel.statusMessage, this.message_panel.additionalMessage, this.message_panel.currentLayer,
    ...
    this.board_panel = new BoardPanel(screen_messages, this, p_locale, p_save_intermediate_stages,
  ...
  boolean load(InputStream p_input_stream, boolean isSpecctraDsn, JTextField p_message_field) {
    ...
    if (isSpecctraDsn) {
      read_result = board_panel.board_handling.loadFromSpecctraDsn(p_input_stream, this.board_observers,
  ...
  public boolean save_intermediate_stage_file() {
    if ((intermediate_stage_file_last_saved_at != null)
        && (intermediate_stage_file_last_saved_at.plusSeconds(30).isAfter(LocalDateTime.now()))) {
      return false;
    }
  ...
  public void set_context_sensitive_help(Component p_component, String p_help_id) {
    ...
    // throw new UnsupportedOperationException("Context sensitive help is
    // disabled.");
  }
